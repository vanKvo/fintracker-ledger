...
/**
 * Outbound Adapter: jOOQ implementation of {@link TransactionRepository}.
 * Uses DSL.noCondition() for dynamic query building — all filters are optional
 * and appended only when present, producing a single efficient SQL query.
 */
public class JooqTransactionRepository implements TransactionRepository {
    ...
    public List<Transaction> findAll(TransactionFilter filter) {
        // JOIN to accounts enforces tenant isolation: only the requesting user's transactions
        // are ever visible, regardless of which accountId is provided.
